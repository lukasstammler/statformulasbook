...
* $SE$ vereinfacht (für Berechnungen von Hand)   
...
3. Bootstrapvertreilung: Die Bootstrapverteilung einer Kennzahl, z.B. $\bar{x}$, repräsentiert die Werte, welche die Kennzahl in allen möglichen Resamples angenommen hat, also die Stichprobenverteilung der Kennzahl.  
...
# 1000 means and medians from 10000samples with replacement --------------------
mean.samples <- vector()
for (i in 1:10000){
...
ggplot(bootstrap, aes(x = m.samples)) +
  ...
  ggtitle(paste("bootstrapped means, samples = 10000, M = ", 
          ...
          subtitle = paste("original mean = ", round(mean(drug), 4)))

# calculate CI -----------------------------------------------------------------
alpha <- 0.05
M <- mean(bootstrap$m.samples)
SE <- sd(bootstrap$m.samples)
CI <- M + c(-1, 1) * abs(qnorm(alpha/2)) * SE
CI

# normalise data to represent the nullhypothesis -------------------------------
drug.norm <- drug - mean(drug)

# 1000 means from 1000 samples with replacement from normalised data -----------
mean.samples <- vector()
for (i in 1:10000){
  sample <- sample(drug.norm, size = length(drug.norm), replace = TRUE)
  mean.samples[i] <- mean(sample)
}
...
ggplot(bootstrap, aes(x = m.samples.norm)) +
  ...
  ggtitle(paste("bootstrapped means, samples = 10000, M = ", 
          ...
          subtitle = paste("boundaries two.sided = ", round(mean(drug), 4)))
...
pnorm(-M.drug) + pnorm(M.drug, lower.tail = FALSE)
```


Example from Frost Hypothesistesting

```{r, eval=FALSE}
bodyfat <- read_csv("body_fat.csv")
psych::describe(bodyfat$PercFat)
hist(bodyfat$PercFat)

# 1000 means and medians from 100000 samples with replacement ------------------
mean.samples <- vector()
median.samples <- vector()
for (i in 1:100000){
  sample <- sample(bodyfat$PercFat, size = length(bodyfat$PercFat), replace = TRUE)
  mean.samples[i] <- mean(sample)
  median.samples[i] <- median(sample)
}
bootstrap <- tibble(
  m.samples = mean.samples,
...
ggplot(bootstrap, aes(x = m.samples)) +
  geom_histogram(fill = "steelblue") +
  ggtitle(paste("bootstrapped means, samples = 100000, M = ", 
                round(mean(bootstrap$m.samples), 4)), 
          subtitle = paste("original mean = ", round(mean(bodyfat$PercFat), 4)))
qqnorm(bootstrap$m.samples)

# plot histogram and median ------------------------------------------------------
ggplot(bootstrap, aes(x = median.samples)) +
  geom_histogram(fill = "steelblue") +
  ggtitle(paste("bootstrapped medians, samples = 100000, Median = ", 
                round(mean(bootstrap$median.samples), 4)), 
          subtitle = paste("original mean = ", round(median(bodyfat$PercFat), 4)))
qqnorm(bootstrap$median.samples)
