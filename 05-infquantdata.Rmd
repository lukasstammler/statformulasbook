...
* **Standardfehler für eine Mittelwertsdifferenz $SE_{\bar{x}_1 - \bar{x}_2}$**    
...
* **$SE_{\bar{x}_1 - \bar{x}_2}$ für eine Mittelwertsdifferenz aus der gepoolten Standardabweichung**  
...
# t-Test für ungleiche Varianzen, R-standard = Welch-Test
...
1. Zufallsstichprobe im Umfang $n$ aus der zu untersuchenden Population ziehen.  
2. Resampling: $B$ Stichproben im Umfang $n$ aus der Zufallsstichprobe ziehen. Dabei wird nach jedem Zug wieder "zurückgelegt", d.h. jede Beobachtungseinheit in der Stichprobe kann mehr als einmal gezogen werden. $B$ hat in der Regel einen Wert von > 10'000.   
3. Bootstrapvertreilung: Die Bootstrapverteilung einer Kennzahl, z.B. $\bar{x}$, repräsentiert die Werte, welche die Kennzahl in allen $B$ Resamples angenommen hat, also die Stichprobenverteilung der Kennzahl.  
...
### Bootstrap-Konfidenzintervall für einen Mittelwert berechnen
...
* Bootstrap Bias-Corrected Accelerated Interval (BCa)  
  ...
  * Das BCa ist eine Modifikation des Perzentilen-CI und sollte immer angewandt werden, wenn die Software dies erlaubt. Der mathematische Hintergrund ist komplex und wird hier nicht weiter erläutert.

Für die Arbeit mit `R` stehen verschiedene packages zur Verfügung:
...
Beispiel: Hat ein neues Medikament einen Effekt auf das Wohlbefinden? Diese Frage wurde an 8 Testpersonen überprüft. Der Effekt auf das Wohlbefinden ist in der Variablen `drug` gespeichert (neg. Werte: Verschlechterung des Wohlbefindens, pos. Werte: Verbesserung des Wohlbefindens)
...
# 10000 means from 10000 samples with replacement ------------------------------
means.boot <- vector()
for (i in 1:10000){
  sample <- sample(drug, size = length(drug), replace = TRUE)
  means.boot[i] <- mean(sample)
}
M.boot <- round(mean(means.boot), 4)  # Mean of bootstrapped means
M.sample <- round(mean(drug), 4)      # Mean of original sample
Bias <- M.boot - M.sample             # Est. bootstrap bias

# plot histogram and mean ------------------------------------------------------
ggplot() +
  geom_histogram(aes(x = means.boot), fill = "steelblue", color = "white") +
  ggtitle(paste("Distribution of bootstrapped means, samples = 10000, mean = ", 
                M.boot), subtitle = paste("Sample mean = ", M.sample,
                           ", Est. bootstrap bias = ",Bias))

# calculate CI -----------------------------------------------------------------
# Bootstrap-z-Intervall
alpha <- 0.05
SE.boot <- sd(means.boot)
CI.z <- M.boot + c(-1, 1) * abs(qnorm(alpha/2)) * SE.boot
CI.z <- round(CI.z, 4)
paste("95%-CI (method norm): [", CI.z[1], ",", CI.z[2], "]")

# Bootstrap-Perzentilen-Intervall
CI.perc <- qnorm(c(.025, .975), 
                 mean = M.boot,
                 sd = sd(means.boot))
CI.perc <- round(CI.perc, 4)
paste("95%-CI (method perc): [", CI.perc[1], ",", CI.perc[2], "]")
```

mit `confintr::ci_mean()` und bca-Methode 

```{r}
confintr::ci_mean(drug, type = "bootstrap", boot_type = "bca", R = 10000, seed = 123)
```


### Bootstrap-Konfidenzintervalle für Gruppenvergleiche   

Prinzip: Gegeben sind unabhängige Zufallsstichproben vom Umfang $n_1$ und $n_2$ von zwei Populationen:  

1. Ziehe ein Resample vom Umfang $n_1$ mit "Zurücklegen" von der ersten Stichprobe und ein separates Resample vom Umfang $n_2$ von der zweiten Stichprobe. Berechne eine Prüfgrösse für den Vergleich der beiden Gruppen (z.B. die Differenz der Mittelwerte).   
2. Wiederhole diesen Vorgang mehrere tausend Male.   
3. Erstelle die Bootstrap-Verteilung der Prüfgrösse. Beurteile die Form, den Bias und $SE_{boot}$ wie oben beschrieben. 

Beispiel: Besteht ein Unterschied zwischen den Schulleistungen gemessen am GPA (Grade Point Average, entspricht dem Notendurchschnitt, Max = 4, Min = 0) von Schülerinnen und Schülern. 

```{r, fig.width=c(5, 8)}
# Source: Moore et al. 2021 ch16 example 16.4
gpa <- read_csv("./eg16-05gpa.csv")
gpa$Sex <- factor(gpa$Sex, levels = c(0,1), labels = c("m", "w"))
...
gpa %>% 
  ...
  kbl(digits = 3, caption = "GPA Kennzahlen") %>% 
  kable_styling(full_width = FALSE)

gpa1_plot <- ggplot2::ggplot(gpa, aes(x = GPA, color = Sex)) +
  geom_density(size = 1) +
  ggtitle("GPA-Scores nach Geschlecht")

gpa2_plot <- ggplot2::ggplot(gpa, aes(sample = GPA, color = Sex)) +
  stat_qq(size = 2, alpha = .6) +
  stat_qq_line() +
  ggtitle("Q-Q-Plot für GPA-Scores nach Geschlecht") +
...
set.seed(123)
for (i in 1:Reps){
  sample.m <- sample(GPA.m, size = n.m, replace = TRUE)
  sample.w <- sample(GPA.w, size = n.w, replace = TRUE)
  mdiff.boot[i] <- mean(sample.m) - mean(sample.w)
}

# Plot bootstrapped mean difference
M.sample <- round(mean(GPA.m) - mean(GPA.w), 4)
M.boot <- round(mean(mdiff.boot), 4)
Bias <- round(M.boot - M.sample, 4)

ggplot2::ggplot() +
  geom_histogram(aes(x = mdiff.boot), fill = "steelblue", color = "white") +
  ggtitle(paste("Bootstrapped GPA mean difference =", M.boot), 
          subtitle = paste("Mean sample =", M.sample, 
                           ", Est. bias = ", Bias))

# Calculate 95%-CI Method norm
z <- qnorm(.975)  
SE.boot <- sd(mdiff.boot)
CI.z <- M.boot + c(-1, 1) * abs(z) * SE.boot
CI.z <- round(CI.z, 4)
paste("95%-CI for mean difference (method norm): [", CI.z[1], ",", CI.z[2], "]")


# Calculate 95%-Ci Method student
df <- min(n.m, n.w) - 1
t <- qt(.975, df = df)  
CI.t <- M.boot + c(-1, 1) * abs(t) * SE.boot
CI.t <- round(CI.t, 4)
paste("95%-CI for mean difference (method student): [", CI.t[1], ",", CI.t[2], "]")

# Calculate 95%-CI Method percentile
CI.perc <- qnorm(c(.025, .975),
                 mean = M.boot,
                 sd = SE.boot)
CI.perc <- round(CI.perc, 4)
paste("95%-CI for mean difference (method perc): [", CI.perc[1], ",", CI.perc[2], "]")

result <- tibble(
  method = c("norm", "stud", "perc"),
  CI.lo = c(CI.z[1], CI.t[1], CI.perc[1]),
  CI.up = c(CI.z[2], CI.t[2], CI.perc[2])
)
result %>% 
  kable(caption = "Bootstrapped CI for Mean difference") %>% 
  kable_styling(full_width = FALSE)
```

mit `confintr::ci_mean_diff()` und bca-Methode

```{r}
confintr::ci_mean_diff(GPA.m, GPA.w, type = "bootstrap", R = 3000, boot_type = "bca")
```
